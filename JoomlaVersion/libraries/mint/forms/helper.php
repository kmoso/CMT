...
class MFormHelper
{
	...
	static public function renderGroup($form, $defaults, $group, $separator = 2, $style = 2)
	{
		...
		foreach($fieldsets as $name => $fieldset)
		{
			/*switch($separator)
			{
				case self::FIELDSET_SEPARATOR_HEADER:
					$out[] = "<h3>" . JText::_($fieldset->label) . "</h3>";
					break;
				case self::FIELDSET_SEPARATOR_FIELDSET:
					if($fieldset->label)
					{
						$out[] = sprintf('<fieldset><legend>%s</legend>', JText::_($fieldset->label));
					}
					break;
			}
			...
			{
				$out[] = '<p><small>' . JText::_($fieldset->description) . '</small><p>';
			}
			*/
		...
		foreach($fields as $key => $field)
		{
			...
			switch($style)
			{
				...
				case self::STYLE_TABLE:
					$full_width = array('Caddress', 'Ccontacts', 'Clinks', 'Cobaltevents');
