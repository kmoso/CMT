(function($){
	$(function(){
		...
		function updateState(self) {
			$.ajax({
				url: self.options.url + 'index.php?option=com_cobalt&task=ajax.get_notifications&tmpl=component&lang=en',
				type: 'post',
				dataType:'json',
				...
				$.each(json.result, function (k, v) {
					if(!notifications.hasOwnProperty(v.id)){
						...
						$('.' + self.options.position).notify({
							...
							onClose: function(){
								$.ajax({
									url: self.options.url + 'index.php?option=com_cobalt&task=ajax.mark_notification&tmpl=component&lang=en',
									type: 'post',
									dataType:'json',
									data:{id: v.id}
								}).done(function(data){
