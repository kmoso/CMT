...
<script type="text/javascript"><!--
	...
	function modJsc_initTree<?php echo $module->id;?>(id, title)
	{
		...
		<?php if($params->get( 'show_section', 1)) :?>
			...
			<?php if($params->get( 'show_section', 1) == 2) :?>
				data: {url: '<?php echo JURI::root(TRUE);?>/index.php?option=com_cobalt&view=records&section_id=<?php echo $section->id;?>&Itemid=<?php echo $Itemid;?>',
						id: id},
			<?php endif;?>
		<?php endif;?>
		});
		...
		<?php if($params->get( 'show_section', 1)) :
			...
			foreach ( $categories as $i => $cat) :
				...
				var node<?php echo $i;?> = tree.insert({
					...
					id:'<?php echo $cat->id;?>',
					open: false,
					data: {url: '<?php echo JURI::root(TRUE);?>/index.php?option=com_cobalt&view=records&section_id=<?php echo $section->id;?>&cat_id=<?php echo $cat->id;?>&Itemid=<?php echo $Itemid;?>',
						id: id}
				});
	...
	{
		jQuery.ajax({
			url: '<?php echo JURI::root(TRUE);?>/index.php?option=com_cobalt&task=ajax.category_records&tmpl=component',
			dataType: 'json',
			type: 'POST',
