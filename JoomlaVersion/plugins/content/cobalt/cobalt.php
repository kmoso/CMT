<?php
/**
 * @copyright	Copyright (C) 2005 - 2012 MintJoomla. All rights reserved.
...
class plgContentCobalt extends JPlugin
{
	...
	public function onContentPrepare($context, &$row, &$params, $page = 0)
	{
		...
		if(!in_array($row->catid, $cats) && !preg_match('/{cobalt-discussion}/iU', $row->text))
        {
			return;
        }

        if(!preg_match('/{cobalt-discussion}/iU', $row->text))
        {
			$row->text .= '{cobalt-discussion}';
        }
		...
		if($this->params->get('rating'))
		{
			...
			$out[] = '<div id="rating-block" class="pull-right">'.JText::_('Total').': '.
				RatingHelp::loadRating($this->params->get('rating_tmpl', 'default'), $result, 0, 0, 'Cobalt.ItemRatingCallBack', 0, 0).'</div>';
		...
		$row->text = str_replace('{cobalt-discussion}', implode("\n", $out), $row->text);
