...
class ContentTemplaterModelItem extends JModelAdmin
{
	...
	public function getItem($pk = null, $getform = false, $getdefaults = false, $group = false)
	{
		...
		if ($group)
		{
			$assignments = $this->removeSettings($item, array('assignto_'));
			$params      = $this->removeSettings($item, array('content', 'jform_', 'customfield'));

			$item = $this->removeSettings($item, array('content', 'assignto_', 'jform_', 'customfield'), false);
	 ...
	 * @since   1.6
	 */
	private function removeSettings($data, $prefixes = array('jform_'), $keep = true)
	{
		$return = new stdClass;
	...
	public function save($data)
	{
		...
		{
			RLText::fixDateOffset($params['assignto_date_publish_up']);
		}

		if (isset($params['assignto_date_publish_down']))
		{
			RLText::fixDateOffset($params['assignto_date_publish_down']);
		}
		...
		if (strpos($str, '[[date:') !== false)
		{
			...
			if (!empty($matches))
			{
				...
				foreach ($matches as $match)
				{
					...
					{
						$match['1'] = RLText::dateToDateFormat($match['1']);
					}
