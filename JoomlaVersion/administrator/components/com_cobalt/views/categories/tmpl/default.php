...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_cobalt&task=categories.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_cobalt&view=categories&section_id='.$section);?>" method="post" name="adminForm" id="adminForm">
	...
	<div id="filter-bar" class="btn-toolbar">
		<div class="filter-search btn-group pull-left">
			<input type="text" placeholder="<?php echo JText::_('CFILTER_SEARCH_CATEGORIESDESC'); ?>" style="margin-left: 5px;" name="filter_search" id="filter_search" value="<?php echo $this->state->get('filter.search'); ?>" />
		</div>
				...
				<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->parent_id;?>" item-id="<?php echo $item->id?>" parents="<?php echo $parentsStr?>"  level="<?php echo $item->level?>">
					...
					<td>
						...
						<?php if ($canEdit || $canEditOwn) : ?>
							<a href="<?php echo JRoute::_('index.php?option=com_cobalt&view=category&task=category.edit&id='.$item->id.'&section_id='.$section);?>">
