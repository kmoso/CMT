...
class CobaltBModelCategory extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 * @since	1.6
	 */
	protected $text_prefix = 'COM_COBALT';
	...
	protected function canDelete($record)
	{
		...
		return $user->authorise('core.delete', 'com_cobalt.category.'.(int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_cobalt.category.'.(int) $record->id);
		}
		// New category, so check against the parent.
		else if (!empty($record->parent_id)) {
			return $user->authorise('core.edit.state', 'com_cobalt.category.'.(int) $record->parent_id);
		}
		// Default to component settings if neither category nor parent known.
		else {
			return $user->authorise('core.edit.state', 'com_cobalt.category');
		}
	}
	 ...
	 * @since	1.6
	*/
	public function getTable($type = 'CobCategory', $prefix = 'CobaltTable', $config = array())
	{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$this->setState('category.section_id', $section);
		$parts = explode('.',$section);

		// Extract the component name
		$this->setState('category.component', 'com_cobalt');
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		$section	= JRequest::getInt('section_id');//$this->getState('filter.section');
		...
		if (empty($section) && isset($data['section'])) {
			...
			$this->setState('category.section',	$section);
			$this->setState('category.component',	'com_cobalt');
			$this->setState('category.section',		@$parts[1]);
		}

		// Get the form.
		$form = $this->loadForm('com_cobalt.category', 'category', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_cobalt.category.edit.'.$this->getName().'.data', array());
	...
	/*protected function preprocessForm(JForm $form, $data, $groups = '')
	{
		...
		echo $component	= $this->getState('category.component');
	...
	*/
	...
	protected function batchAccess($value, $pks, $contexts)
	{
		...
		$section = JRequest::getInt('section_id');
		foreach ($pks as $pk) {
			if (!$user->authorise('core.edit', 'com_cobalt.category.'.$pk)) {
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		if ($parentId) {
			...
			$canCreate = ($parentId == $table->getRootId()) ? $user->authorise('core.create', $section) :
				$user->authorise('core.create', 'com_cobalt.category.'.$parentId);
			if (!$canCreate) {
	...
	{
		// Alter the title & alias
		$catTable = JTable::getInstance('CobCategory', 'CobaltTable');
