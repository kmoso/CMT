...
foreach($this->value as $picture_index => $file)
{
	...
	if($rel)
	{
		...
		if($i <= 2) {
			$out[] = '<a href="'.$url_orig.'" class="image-wrapper-thumb" '.$rel.' id="'.$array_keys[$file['filename']].'"><img src="'.$url.'" class="img-polaroid"  alt=""></a>';
		} else {
			$out2[] = '<a href="'.$url_orig.'" class="image-wrapper-thumb" '.$rel.' id="'.$array_keys[$file['filename']].'"><img src="'.$url.'" class="img-polaroid"  alt=""></a>';
		}
	}
	else
	{
