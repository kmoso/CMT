	...
	<div style="margin: 15px 0px">
		...
		<?php foreach ($this->markers AS $marker):?>
			<img id="mrk-<?php echo md5($marker);?>" class="filter-marker<?php if($default->get('marker.name') == $marker){$active = $marker; echo ' active';}?>" onclick="filterMarker<?php echo $this->id  ?>(this, '<?php echo $marker;?>')" class="hasTip" title="<?php echo $this->_getMarkerName($marker)?>"
				 src="<?php echo JURI::root(TRUE)?>/components/com_cobalt/fields/geo/markers/<?php echo $this->params->get('params.map_icon_src.dir', 'custom');?>/<?php echo $marker;?>" >
		<?php endforeach;?>
	</div>
<?php endif;?>
...
<script type="text/javascript">
	(function($) {
		...
		var markerIcon = new google.maps.MarkerImage('<?php echo JUri::root();?>components/com_cobalt/fields/geo/markers/grouped/other/marker-big-blue.png', msize, new google.maps.Point(0,0), mpoint);
		...
		window.getChilds = function(val, child)
		{
			...
			$.ajax({
				url : '<?php echo JRoute::_('index.php?option=com_cobalt&task=ajax.field_call&tmpl=component', FALSE) ;?>',
				dataType: 'json',
				type: 'POST',
				data : {
