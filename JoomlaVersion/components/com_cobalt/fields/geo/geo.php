...
class JFormFieldCGeo extends CFormField
{
	...
	public function getInput()
	{
		if(in_array($this->params->get('params.map_marker'), $this->user->getAuthorisedViewLevels()))
		{
			...
			JFactory::getDocument()->addScript(JUri::root(TRUE) . '/components/com_cobalt/fields/geo/assets/geo.js');
		}

		$this->user    = JFactory::getUser();
		$this->map_key = JComponentHelper::getParams('com_cobalt')->get('map_key');
	...
	public function onFilterWornLabel($section)
	{
		...
		if(in_array($this->params->get('params.filter_marker', 0), $user->getAuthorisedViewLevels()))
		{
			...
			{
				$out = '<img align="absmiddle" src="' . JURI::root(TRUE) . '/components/com_cobalt/fields/geo/markers/' . $this->params->get('params.map_icon_src.dir', 'google') . '/' . $default->get('marker.name') . '" />';
	...
	public static function getAddressFields()
	{
		return array(
			...
			'person'   => array(
				...
				'icon'  => JURI::root() . 'components/com_cobalt/fields/geo/icons/person.png'
			),
			'address1' => array(
	...
	public static function getAditionalFields()
	{
		$out = array(
			'tel'    => array(
				'label'  => JText::_('G_TEL'),
				'patern' => '[VALUE]',
				'icon'   => JURI::root() . 'components/com_cobalt/fields/geo/icons/phone.png',
				'preg'   => '[0-9\(\)\. \-\+\#]*'
			),
			'mob'    => array(
				'label'  => JText::_('G_MOBILE'),
				'patern' => '[VALUE]',
				'icon'   => JURI::root() . 'components/com_cobalt/fields/geo/icons/mobile.png',
				'preg'   => '[0-9\(\)\. \+\#]*'
			),
			'fax'    => array(
				'label'  => JText::_('G_FAX'),
				'patern' => '[VALUE]',
				'icon'   => JURI::root() . 'components/com_cobalt/fields/geo/icons/fax.png',
				'preg'   => '[0-9\(\)\. \-\+\#]*'
			),
			'icq'    => array(
				'label'  => JText::_('G_ICQ'),
				'patern' => '<a href="http://www.icq.com/people/[VALUE]#pager">[VALUE]</a>',
				'icon'   => JURI::root() . 'components/com_cobalt/fields/geo/icons/icq.png',
				'preg'   => '[0-9]*'
			),
			'skype'  => array(
			...
			'google' => array(
				...
				'patern' => '[VALUE]',
				'icon'   => JURI::root() . 'components/com_cobalt/fields/geo/icons/google.png'
			),
			'yahoo'  => array(
				'label'  => JText::_('G_YAHOOMSG'),
				'patern' => '<a href="ymsgr:sendim?[VALUE]">[VALUE]</a>',
				'icon'   => JURI::root() . 'components/com_cobalt/fields/geo/icons/yahoo.png'
			),
			'msn'    => array(
				'label'  => JText::_('G_MSN'),
				'patern' => '<a href="msnim:chat?contact=[VALUE]">[VALUE]</a>',
				'icon'   => JURI::root() . 'components/com_cobalt/fields/geo/icons/msn.png',
				'filter' => FILTER_VALIDATE_EMAIL
			),
			'aol'    => array(
				'label'  => JText::_('G_AOL'),
				'patern' => '<a href="aim:goim?screenname=[VALUE]">[VALUE]</a>',
				'icon'   => JURI::root() . 'components/com_cobalt/fields/geo/icons/aol.png'
			)
		);
	...
	public static function getAditionalinks()
	{
		$out = array(
			'web'         => array(
				...
				'icon'  => JURI::root() . 'components/com_cobalt/fields/geo/icons/web.png'
			),
			'facebook'    => array(
				'label' => JText::_('G_FACEBOOK'),
				'icon'  => JURI::root() . 'components/com_cobalt/fields/geo/icons/fb.png'
			),
			'twitter'     => array(
				'label' => JText::_('G_TWITTER'),
				'icon'  => JURI::root() . 'components/com_cobalt/fields/geo/icons/twitter.png'
			),
			'linkin'      => array(
				'label' => JText::_('G_LINKIN'),
				'icon'  => JURI::root() . 'components/com_cobalt/fields/geo/icons/linkedin.png'
			),
			'google'      => array(
				'label' => JText::_('G_GOOGLEPLUS'),
				'icon'  => JURI::root() . 'components/com_cobalt/fields/geo/icons/google-plus.png'
			),
			'youtube'     => array(
				'label' => JText::_('G_YOUTUBE'),
				'icon'  => JURI::root() . 'components/com_cobalt/fields/geo/icons/youtube.png'
			),
			'odnoclasnik' => array(
				'label' => JText::_('G_ODNOKLASSNIKI'),
				'icon'  => JURI::root() . 'components/com_cobalt/fields/geo/icons/od.png'
			)
		);
	...
	protected function _getList(&$post, $label = 'G_SELECTCOUNTRY')
	{
		...
		switch($index)
		{
			case 'country':
				...
				$dir  = JPATH_ROOT . '/components/com_cobalt/fields/geo/countries/';
	...
	{
		$icon_url = '/components/com_cobalt/fields/geo/markers/google/marker-big-red.png';

		if(!empty($this->value['position']['marker']))
		{
			$icon_url = '/components/com_cobalt/fields/geo/markers/' . $this->params->get('params.map_icon_src.dir', 'custom') . '/' . $this->value['position']['marker'];
		}
	...
	public function onInfoWindow($post, $record)
	{
		...
		$this->field_keys_by_id = JModelLegacy::getInstance('Records', 'CobaltModel')->getKeys($section);
		...
		$tpath    = JPATH_THEMES . '/' . $template . '/html/com_cobalt/fields/' . $this->type . '/window/' . $this->params->get('params.template_window', 'default.php');
		if(!JFile::exists($tpath))
		{
			$tpath = JPATH_ROOT . '/components/com_cobalt/fields/' . $this->type . '/tmpl/window/' . $this->params->get('params.template_window', 'default.php');
		}
		include $tpath;
		$out = ob_get_contents();
		ob_end_clean();
