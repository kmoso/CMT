...
class JFormFieldCStatus extends CFormField
{
	...
	private function _getStatusView($client, $record, $type, $section)
	{
		...
		if(isset($val[1]))
		{
			$value_color = '<span style="color: ' . $val[1] . '">' . $value . '</span>';
		}
		...
		{
			case 1 :
				$out[] = JHtml::image($path . $params->get('params.icon' . $this->value), $value, array(
					'class' => 'hasTip',
					'title' => $value,
					'align' => 'absmiddle'
				));
	...
	public function _changeStatus($post)
	{
		...
		$table_rec = JTable::getInstance('Record', 'CobaltTable');
