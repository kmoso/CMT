...
{
	?>
	<select name="status"  style="max-width:150px;" class="inputbox" onchange="Cobalt.changeStatus(this, <?php echo $o->id?>)">
...
!function($)
{
	Cobalt.changeStatus = function (select, order)
	{
		var b = $('#bar_' + order);
		b.slideDown('quick', function(){
			$.ajax({
				url: '<?php echo JRoute::_("index.php?option=com_cobalt&task=ajax.status&tmpl=component", FALSE); ?>',
				dataType: 'json',
				type: 'POST',
				data:{
					order_id:order,
...
<form action="<?php echo JRoute::_('index.php?option=com_cobalt&view=elements&layout=saler&Itemid='.JFactory::getApplication()->input->getInt('Itemid')); ?>" method="post" name="adminForm" id="sales-form">
	...
	<div class="controls controls-row">
		...
		<button class="btn pull-right" onclick="window.location ='<?php echo JRoute::_('index.php?option=com_cobalt&view=elements&layout=addsale'); ?>';return false;">
			...
			<tr class=" <?php echo $k = 1 - @$k?>">
				<td>
					<?php if($order->gateway == 'CMANUAL' || !$order->gateway):?>
						<a href="javascript:void(0)" onclick="if(confirm('<?php echo JText::_('CCONFIRMDELET_1')?>')){window.location = '<?php echo JRoute::_('index.php?option=com_cobalt&task=sale.delete&id='.$order->id)?>'}">
					...
					<a onclick="Cobalt.setAndSubmit('filter_search', 'pid:<?php echo $order->record_id;?>');" href="javascript:void(0);" rel="tooltip" data-original-title="<?php echo JText::_('CORDERSPRODFILTER')?>">
						...
						<table class="table table-bordered table-condenced table-stripped">
							<?php if($order->saler_id != $userId):?>
								<tr>
									...
									<td><?php echo CCommunityHelper::getName($order->saler_id, $order->section_id)?>
										<a onclick="Cobalt.setAndSubmit('filter_search', 'sid:<?php echo $order->saler_id;?>');" href="javascript:void(0);" rel="tooltip" data-original-title="<?php echo JText::_('CORDERSSALERFILTER')?>">
											<?php echo HTMLFormatHelper::icon('funnel-small.png');  ?></a>
									</td>
								</tr>
							<?php endif;?>
							<tr>
								...
								<td><?php echo CCommunityHelper::getName($order->user_id, $order->section_id)?>
									<a onclick="Cobalt.setAndSubmit('filter_search', 'bid:<?php echo $order->user_id;?>');" href="javascript:void(0);" rel="tooltip" data-original-title="<?php echo JText::_('CORDERSBUYERFILTER')?>">
										<?php echo HTMLFormatHelper::icon('funnel-small.png');  ?></a>
								</td>
							</tr>
							<tr>
