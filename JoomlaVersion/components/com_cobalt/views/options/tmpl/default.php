...
<div class="page-header">
	<button class="btn pull-right" type="button" onclick="window.location = '<?php echo JRoute::_('index.php?option=com_cobalt&view=notifications')?>'">
		...
		<ul class="nav nav-tabs nav-stacked">
			<?php foreach ($this->usub AS $k => $s):?>
				<li <?php echo ($s->id == $app->input->getInt('section_id') ? 'class="active"' : NULL)?>>
					<a href="<?php echo JRoute::_('index.php?option=com_cobalt&view=options&section_id='.$s->id.'&return='.$app->input->getString('return'));?>"><?php echo $s->name;?></a>
				</li>
			<?php endforeach;?>
		</ul>
			...
			<?php if(!empty($this->section->id)):?>
				...
				<div class="tab-content well" id="tabs-content">
					...
					<div class="tab-pane" id="page-recfollows">
						...
						<div class="clearfix"></div>
						<div class="pull-right">
							<button type="button" <?php echo ($section->records ? NULL : 'style="display: none;"');?> class="btn" id="unsub-<?php echo $section->id?>" onclick="Cobalt.ajax_unsubscr_sec(<?php echo $section->id;?>)">
								<?php echo JText::_('CUNSUBREC')?>
								<span class="badge"><?php echo $section->records;?></span>
							</button>
							<button class="btn" type="button" <?php echo (($section->records_total && ($section->records_total > $section->records)) ? NULL : 'style="display: none;"');?> id="sub-<?php echo $section->id?>" onclick="Cobalt.ajax_subscr_sec(<?php echo $section->id;?>)" href="javascript:void(0);">
...
!function($)
{
	Cobalt.ajax_unsubscr_sec = function (id)
	{
		if(!confirm('<?php echo JText::_('CSURE')?>')) return;
		$('#bro-ba').slideDown('quick', function(){
			$.ajax({
				url: '<?php echo JRoute::_('index.php?option=com_cobalt&task=ajax.unfollowallsection&tmpl=component', FALSE); ?>',
				dataType: 'json',
				type: 'POST',
				data:{
					section_id:id
	...
	Cobalt.ajax_subscr_sec = function (id)
	{
		$('#bro-ba').slideDown('quick', function(){
			$.ajax({
				url: '<?php echo JRoute::_('index.php?option=com_cobalt&task=ajax.followallsection&tmpl=component', FALSE); ?>',
				dataType: 'json',
				type: 'POST',
				data:{
					section_id:id
