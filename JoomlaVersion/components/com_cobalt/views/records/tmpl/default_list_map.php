...
$fieldtable = JTable::getInstance('Field', 'CobaltTable');
...
{
	$size = getimagesize(JPATH_ROOT . '/components/com_cobalt/fields/geo/markers/' . $markerfolder . '/' . $fieldparams->get('params.map_icon_src.icon'));
...
{
	JFactory::getDocument()->addScript('//google-maps-utility-library-v3.googlecode.com/svn/tags/markerclustererplus/2.0.15/src/markerclusterer_packed.js');
}
...
(function($) {
	...
	var mapStyle =    <?php include JPATH_ROOT. DIRECTORY_SEPARATOR .'components/com_cobalt/library/js/mapstyles/'.$params->get('tmpl_params.map_style'); ?>;
	map.setOptions({styles: mapStyle});
	<?php endif;?>

	$.ajax({
		url:  Cobalt.field_call_url, dataType: 'json', type: 'POST',
		data: {
		...
		markerCluster = new MarkerClusterer(map, markers, {gridSize: <?php echo $params->get('tmpl_params.map_claster_size', 40); ?>, maxZoom: 15});
