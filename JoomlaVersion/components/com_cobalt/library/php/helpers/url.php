...
class Url
{
	...
	{
		$url = 'index.php?option=com_cobalt&task=form.edit&id=' . $id . '&return=' . ($return ? $return : self::back());
	...
	static public function task($task, $id, $return = FALSE)
	{
		...
		$url = 'index.php?option=com_cobalt&task=' . $task . '&id=' . $id . '&return=' . $r;
	...
	{
		$url = 'index.php?option=com_cobalt';
	...
	static public function records($section, $category = NULL, $user_id = NULL, $vw = NULL)
	{
		...
		$url = 'index.php?option=com_cobalt&view=records&section_id=';
		...
		if($itemid && $url == 'index.php?option=com_cobalt&view=records&section_id='.$section->id . COBS . $section->alias)
		{
