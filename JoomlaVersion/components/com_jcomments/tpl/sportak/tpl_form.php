...
?>
<h3 class="title-bottom">Deja un <span>Comentario</span></h3>
<?php
...
<form id="comments-form" name="comments-form" action="javascript:void(null);">
    <div class="uk-grid">
        <?php if (in_array(8, JFactory::getUser()->groups)){
            $tt_class = "uk-width-1-1";
        }else{
            $tt_class = "uk-width-1-2";
        }?>
        <div class="<?php echo $tt_class; ?> uk-panel"> 
            <?php $this->getFormFields($htmlFormPrepend); ?>
            
            
            <?php if ($this->getVar( 'comments-form-user-name', 1) == 1) : ?>
               <?php $text = ($this->getVar('comments-form-user-name-required', 1) == 0) ? JText::_('FORM_NAME') : JText::_('FORM_NAME_REQUIRED'); ?>   
                <p>
                    <span>
                        <input id="comments-form-name" type="text" placeholder="Name" name="name" value="" maxlength="<?php echo $this->getVar('comment-name-maxlength');?>" size="22" tabindex="1" />
                    </span>
                </p>
            <?php endif; ?>
		...
		<?php $text = ($this->getVar('comments-form-email-required', 1) == 0) ? JText::_('FORM_EMAIL') : JText::_('FORM_EMAIL_REQUIRED'); ?>
                <p>
                    <span>
                            <input id="comments-form-email" type="text" placeholder="Email" name="email" value="" size="22" tabindex="2" />
                    </span>
                </p>
            <?php endif; ?>
                
            <?php if ($this->getVar('comments-form-user-homepage', 0) == 1) : ?>
                <?php $text = ($this->getVar('comments-form-homepage-required', 1) == 0) ? JText::_('FORM_HOMEPAGE') : JText::_('FORM_HOMEPAGE_REQUIRED'); ?>
                <p>
                    <span>
                            <input id="comments-form-homepage" type="text" placeholder="Website" name="homepage" value="" size="22" tabindex="3" />
                    </span>
                </p>
            <?php endif; ?>
        </div>
        <div class="<?php echo $tt_class; ?> uk-panel uk-flex uk-flex-column">
            <?php if ($this->getVar('comments-form-title', 0) == 1) : ?>
		<?php $text = ($this->getVar('comments-form-title-required', 1) == 0) ? JText::_('FORM_TITLE') : JText::_('FORM_TITLE_REQUIRED'); ?>
                <p>
                    <span>
...
<?php if ($this->getVar('comments-form-captcha', 0) == 1) {
